...
class IconExtractor():
    ...
    get_group_icon_entries = _get_group_icon_entries
    ...
    def _write_ico(self, fd, num=0):
        ...
        icon_images, sizes = self._get_icon_data([g.ID for g in group_icons])
    ...
    def get_iconb(self, num=0, index=0, biggest=False):
        group_icons = self._get_group_icon_entries(num=num)
        icon_images, sizes = self._get_icon_data([g.ID for g in group_icons])
        if biggest:
            for i in icon_images:
                if b"PNG" in i:
                    return icon_images[icon_images.index(i)]

            self.export_icon("temp.ico", num)
            file = open("temp.ico", "rb")
            img = file.read()
            file.close()
            return img

        else:
            return icon_images[index]

    def get_length(self, num=0):
        return len(self._get_group_icon_entries(num=num))
