...
def get_image_fill(img: Image.Image, size: (0, 0)):
    i_size = img.size
    i_ratio = i_size[0] / i_size[1]
    ratio = size[0] / size[1]

    if i_ratio >= ratio:
        # keep height
        height = size[1]
        width = round(height * i_ratio)
        x = round((width-size[0])/2)
        y = 0

    else:
        # keep width
        width = size[0]
        height = round(width / i_ratio)
        x = 0
        y = round((height-size[1])/2)

    new_img = img.resize((width, height))
    final_img = new_img.crop((x, y, x+size[0], y+size[1]))
